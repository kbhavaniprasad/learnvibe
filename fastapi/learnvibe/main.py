...
def generate_skill_specific_questions(skill_data: Dict, num_questions: int = 5) -> List[Dict]:
...
def generate_questions_with_ollama(skill_or_topic_name: str, context: str, num_questions: int = 5) -> List[Dict]:
...
# ============================================================================
# ROADMAP GENERATION ENDPOINTS (Updated for AI-Driven Generation)
# ============================================================================
...
async def generate_roadmap_endpoint(request: dict):
    """Generate optimized AI-driven learning roadmap"""
    try:
        ...
        total_learning_items = len(weak_skills) + len(weak_topics)
        if total_learning_items == 0:
            raise HTTPException(
                status_code=400, 
                detail="Learning stack is empty. Complete assessments first."
            )
        
        logger.info(f"🚀 FAST Roadmap Generation: {user_goal}, {total_learning_items} items")
        ...
        try:
            ...
            logger.error("Roadmap generation timed out after 1 minute")
            # Return fast fallback
            roadmap_data = roadmap_generator._generate_fast_fallback({
                "user_goal": user_goal,
                "current_level": current_level,
                "target_level": target_level,
                "assessment_score": assessment_score,
                "learning_stack": weak_skills + weak_topics,
        ...
        SESSION_STORE[session_id].update({
            ...
            "roadmap_generated_at": datetime.now().isoformat(),
            "generation_time": time.time() - start_time
        })
        
        logger.info(f"✅ FAST Roadmap Complete: {time.time() - start_time:.1f}s")
...
@app.post("/api/generate-fast-roadmap")
async def generate_fast_roadmap_endpoint(request: dict):
    """Ultra-fast roadmap generation with minimal AI"""
    try:
        start_time = time.time()
        ...
        learning_stack = weak_skills + weak_topics
        if not learning_stack:
            raise HTTPException(status_code=400, detail="No learning items provided")
        
        # Use fast fallback directly with proper integer calculation
        roadmap_data = roadmap_generator._generate_fast_fallback({
            "user_goal": user_goal,
            ...
            "total_weeks": int(min(12, max(4, len(learning_stack))))  # Ensure integer
        })
        ...
        SESSION_STORE[session_id]["fast_generated"] = True
        SESSION_STORE[session_id]["generation_time"] = time.time() - start_time
        
        logger.info(f"⚡ ULTRA-FAST Roadmap: {time.time() - start_time:.2f}s")
        
        return roadmap_data
        
    except Exception as e:
        logger.error(f"Fast roadmap error: {str(e)}")
        raise HTTPException(status_code=500, detail="Fast roadmap generation failed")

@app.post("/api/regenerate-roadmap/{session_id}")
async def regenerate_roadmap(session_id: str, request: dict = None):
    """Regenerate roadmap with different timeline or refresh"""
    try:
        if session_id not in SESSION_STORE:
            raise HTTPException(status_code=404, detail="Session not found")
        ...
        if request and "timeline_preference" in request:
            ...
            timeline_preference = session.get("timeline_preference", "standard")
        ...
        assessment_data = {
            ...
            "current_level": session.get("current_level") or old_roadmap.get("current_level", "Beginner Level"),
            "target_level": session.get("target_level") or old_roadmap.get("target_level", "Advanced Level"),
            "assessment_score": float(session.get("assessment_score", 50)),
        ...
        # Validate learning stack
        if not assessment_data["weak_skills"] and not assessment_data["weak_topics"]:
            raise HTTPException(
                status_code=400,
                detail="Cannot regenerate roadmap without learning stack. Complete assessments first."
            )
        
        logger.info(f"Regenerating AI roadmap for session {session_id} with timeline: {timeline_preference}")
        ...
        SESSION_STORE[session_id]["regeneration_count"] = session.get("regeneration_count", 0) + 1
        
        uniqueness = new_roadmap.get("uniqueness_metrics", {})
        logger.info(f"Roadmap regenerated: {len(new_roadmap.get('weekly_schedule', []))} weeks, "
                   f"{uniqueness.get('unique_tasks', 0)} unique tasks ({uniqueness.get('uniqueness_percentage', 0):.1f}%)")
...
async def update_task_status(session_id: str, task_update: dict):
    ...
    try:
        ...
        # Validate indices
        if week_idx is None or day_idx is None or task_idx is None:
            raise HTTPException(status_code=400, detail="Missing task indices")
        
        weekly_schedule = roadmap.get("weekly_schedule", [])
        
        if (week_idx >= len(weekly_schedule) or
            day_idx >= len(weekly_schedule[week_idx].get("days", [])) or
            task_idx >= len(weekly_schedule[week_idx]["days"][day_idx].get("tasks", []))):
            raise HTTPException(status_code=400, detail="Invalid task indices")
        
        # Update task status
        task = weekly_schedule[week_idx]["days"][day_idx]["tasks"][task_idx]
        ...
        SESSION_STORE[session_id]["roadmap"] = roadmap
        
        logger.info(f"Task {task.get('title', 'Unknown')} marked as {'completed' if completed else 'incomplete'}")
...
async def get_roadmap_progress(session_id: str):
    """Get detailed roadmap progress statistics"""
    try:
        if session_id not in SESSION_STORE:
            raise HTTPException(status_code=404, detail="Session not found")
        ...
        for week in roadmap.get("weekly_schedule", []):
            ...
            phase_name = week.get("learning_phase", "Unknown")
            if phase_name not in phase_progress:
                phase_progress[phase_name] = {"total": 0, "completed": 0}
            
            for day in week.get("days", []):
                for task in day.get("tasks", []):
                    total_tasks += 1
                    week_total += 1
                    phase_progress[phase_name]["total"] += 1
                    
                    if task.get("completed", False):
                        completed_tasks += 1
                        week_completed += 1
                        phase_progress[phase_name]["completed"] += 1
            
            week_progress.append({
                "week_number": week.get("week_number"),
                "week_title": week.get("title", f"Week {week.get('week_number')}"),
                "phase": phase_name,
                "total_tasks": week_total,
                "completed_tasks": week_completed,
                "progress_percentage": round((week_completed / week_total * 100), 1) if week_total > 0 else 0
            })
        
        # Calculate phase progress percentages
        for phase_name, stats in phase_progress.items():
        ...
        if completed_tasks > 0:
            days_passed = sum(1 for w in week_progress for d in range(5) if d < w["completed_tasks"])
            avg_tasks_per_day = completed_tasks / max(days_passed, 1)
            remaining_tasks = total_tasks - completed_tasks
            estimated_days_remaining = remaining_tasks / avg_tasks_per_day if avg_tasks_per_day > 0 else 0
            estimated_completion = (datetime.now() + timedelta(days=estimated_days_remaining)).isoformat()
        else:
            estimated_completion = roadmap.get("metadata", {}).get("estimated_end_date")
        ...
        return {
            ...
            "remaining_tasks": total_tasks - completed_tasks,
            "current_week": max((i + 1 for i, w in enumerate(week_progress) if w["completed_tasks"] < w["total_tasks"]), default=len(week_progress)),
            "total_weeks": roadmap.get("total_weeks", 0),
            "week_progress": week_progress,
            "phase_progress": phase_progress,
            "uniqueness_metrics": roadmap.get("uniqueness_metrics", {}),
            "learning_stack_size": roadmap.get("learning_stack_size", 0),
            "estimated_completion_date": estimated_completion,
            "ai_generated": roadmap.get("metadata", {}).get("ai_generated", False),
            "template_based": roadmap.get("metadata", {}).get("template_based", True)
        }
...
async def delete_roadmap(session_id: str):
    ...
    try:
        if session_id in SESSION_STORE:
            roadmap = SESSION_STORE[session_id].get("roadmap")
            roadmap_info = {
                "weeks": len(roadmap.get("weekly_schedule", [])) if roadmap else 0,
                "ai_generated": roadmap.get("metadata", {}).get("ai_generated", False) if roadmap else False
            }
            
            del SESSION_STORE[session_id]
            logger.info(f"Roadmap deleted for session {session_id}")
...
@app.get("/api/roadmap/validate-uniqueness/{session_id}")
async def validate_roadmap_uniqueness(session_id: str):
    """Validate task uniqueness in roadmap"""
    try:
        if session_id not in SESSION_STORE:
            raise HTTPException(status_code=404, detail="Session not found")
        ...
        for week in roadmap.get("weekly_schedule", []):
            for day in week.get("days", []):
                for task in day.get("tasks", []):
                    ...
                    all_tasks.append(task_title)
                    
                    if task_title in task_titles:
                        duplicates.append({
                            "title": task["title"],
                            "original_location": task_titles[task_title],
                            "duplicate_location": {
        ...
        return {
            ...
            "duplicates": duplicates[:10],  # Return first 10 duplicates
            "validation_passed": uniqueness_percentage >= 95,
            "quality_rating": "excellent" if uniqueness_percentage >= 95 else 
                            "good" if uniqueness_percentage >= 85 else
                            "fair" if uniqueness_percentage >= 70 else "poor",
            "ai_generated": roadmap.get("metadata", {}).get("ai_generated", False),
            "template_based": roadmap.get("metadata", {}).get("template_based", True)
        }
        ...
        raise HTTPException(status_code=500, detail="Failed to validate uniqueness")


@app.get("/api/roadmap/learning-stack/{session_id}")
async def get_learning_stack_info(session_id: str):
    """Get information about the learning stack used to generate roadmap"""
    try:
        if session_id not in SESSION_STORE:
            raise HTTPException(status_code=404, detail="Session not found")
        ...
        weak_skills = SESSION_STORE[session_id].get("weak_skills", [])
        weak_topics = SESSION_STORE[session_id].get("weak_topics", [])
        strong_areas = SESSION_STORE[session_id].get("strong_areas", [])
        
        return {
            "total_items": len(learning_stack),
            ...
            "learning_stack": learning_stack,
            "coverage_analysis": {
                "skills_addressed": len(weak_skills),
                "topics_addressed": len(weak_topics),
                "total_areas_to_master": len(learning_stack)
            }
        }
        ...
        logger.error(f"Learning stack info error: {str(e)}")
        raise HTTPException(status_code=500, detail="Failed to get learning stack info")


@app.get("/api/roadmap/week-detail/{session_id}/{week_number}")
async def get_week_detail(session_id: str, week_number: int):
    ...
    try:
        ...
        week = next((w for w in weekly_schedule if w.get("week_number") == week_number), None)
        
        if not week:
            raise HTTPException(status_code=404, detail=f"Week {week_number} not found")
        
        # Calculate week statistics
        total_tasks = sum(len(day.get("tasks", [])) for day in week.get("days", []))
        completed_tasks = sum(
            sum(1 for task in day.get("tasks", []) if task.get("completed", False))
            for day in week.get("days", [])
        )
        ...
        return {
            **week,
            "statistics": {
                ...
                "total_days": len(week.get("days", [])),
                "estimated_hours": len(week.get("days", [])) * roadmap.get("metadata", {}).get("daily_study_hours", 2)
            }
        }
        ...
        logger.error(f"Week detail error: {str(e)}")
        raise HTTPException(status_code=500, detail="Failed to get week details")


@app.post("/api/roadmap/export/{session_id}")
async def export_roadmap(session_id: str, format: str = "json"):
    ...
    try:
        ...
        if format.lower() == "json":
            ...
            summary = {
                ...
                "duration": f"{roadmap.get('total_weeks')} weeks",
                "total_tasks": sum(
                ...
                "phases": [
                    {
                        "name": phase.get("name"),
                        "duration": f"{phase.get('duration')} weeks",
                        "focus_areas": phase.get("focus_areas", [])[:5]
                    }
            ...
            raise HTTPException(status_code=400, detail="Unsupported export format. Use 'json' or 'summary'")
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Export error: {str(e)}")
        raise HTTPException(status_code=500, detail="Failed to export roadmap")
